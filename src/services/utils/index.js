...
export const getDifferenceTimes = (totalWorkDay, totalWorkedHours, totalMonthHours, data) => {
    ...
    const remainHoursTotal = leftHours.map((item, index) => {
        if ((index === 1 || index === 2) && (item - totalCurrentTimes[index]) < 0) {
            return (item - totalCurrentTimes[index]) * -1
        } else {
            return item - totalCurrentTimes[index]
        }
    })
